...
/*   Updated: 2023/08/31 18:26:07 by nelallao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	casting_horizontal(t_cub3d *mlx, double ray_angle, t_ray *s)
{
	...
	while (s->next_horzintal_x >= 0
		&& s->next_horzintal_x <= TILE_SIZE * mlx->colons
		&& s->next_horzintal_y >= 0
		&& s->next_horzintal_y <= s->next_horzintal_y <= TILE_SIZE * mlx->rows)
	{
		if (ft_wall_here_a(s->next_horzintal_x,
				s->next_horzintal_y - c.up, mlx))
		{
...
void	casting_vertical(t_cub3d *mlx, double ray_angle, t_ray *s)
{
	...
	while (s->nx_vert_x >= 0 && s->nx_vert_x <= TILE_SIZE * mlx->colons
		&& s->next_vertical_y >= 0 && s->next_vertical_y <= TILE_SIZE * mlx->rows)
	{
...
void	ft_hor_intersection(t_casting *c, float angle, t_cub3d *mlx, t_ray *s)
{
	...
	c->x_inter = mlx->player->x + (c->y_inter - mlx->player->y) / tan(angle);
	c->y_step = TILE_SIZE;
	if (c->up)
		c->y_step *= (-1);
